...
* First three convolutional layers with kernel size 5x5 are enough for extracting features of three different scales from the original images. The depth of the first layers 24-36-48 must provide enough capacity for all possible low-level features seen on the track.
* The following two high-depth convolutional layers should be able to extract pretty complex features out from the third high-scale feature layer. The capacity of these two layers is 64-64. 
...
* L2-regularization is applied to all weights with lambda=1e-5. A higher value of lambda would affect the training time with not much effect on the generalization.
* Dropout is applied to all fully-connected neurons. It demonstrated a significant improvement in the final loss and the overall model performance on track. The dropout probability is chosen 0.5
...
The batch size is chosen 256. A larger batch size of 512 or 1024 would require a really powerful GPU and reduce number of back propagations per epoch. The base number of epochs is 15, however the model was additionally trained at a lower learning rate after the initial testing on track.
...
1. Camera is randomly selected from the center, left or right camera. If a side camera is selected, the original angle is adjusted by 0.25.
...
### Validation Set
The validation set is randomly created by the same generator from the original samples without discarding any straight driving samples. The validation set size is 20% of the training set size.
